    ...
    <div class="flex mt-[70px]">
        <div class="w-full px-3">
            ...
            <form method="POST" action="/izin/{{ $izin->id }}/update" id="form_izin" enctype="multipart/form-data" autocomplete="off">
                ...
                <div id="jamDatangWrapper" class="mb-4" style="display:none;">
                    <label class="text-[12px] font-semibold text-[#44403c] mb-1 block">
...
@push('myscript')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Kategori help text + jam datang toggle
            var jenisSelect = document.getElementById('jenis_izin');
            var jenisHelp = document.getElementById('jenisHelp');
            var jamDatangWrapper = document.getElementById('jamDatangWrapper');
            var jamDatangSelect = document.getElementById('jam_datang');
            var helpTexts = {
                'tidak_masuk': 'Anda mengajukan izin tidak masuk kerja pada tanggal yang dipilih.',
                'terlambat': 'Anda mengajukan izin terlambat masuk kerja. Pilih jam Anda akan datang.',
                'setengah_hari': 'Anda mengajukan izin setengah hari.',
            ...
            function toggleJamDatang() {
                if (jenisSelect.value === 'terlambat') {
                    jamDatangWrapper.style.display = '';
                    jamDatangSelect.setAttribute('required', 'required');
                } else {
                    jamDatangWrapper.style.display = 'none';
                    jamDatangSelect.removeAttribute('required');
                    jamDatangSelect.value = '';
                }
            }
            ...
            document.getElementById('form_izin').addEventListener('submit', function(e) {
                ...
                if (jenis === 'terlambat' && !jamDatangSelect.value) {
                    Swal.fire({ icon: "warning", text: "Jam datang harus dipilih untuk izin terlambat", confirmButtonColor: "#7a5234" });
                    return;
                }
                if (!tgl) {
